...
Status: the 2026-09-12 integration uses 120 Hz GELLO acquisition, 120 Hz URSim host sends and independent 125 Hz feedback. Software checks pass (450 native; 453 Mac installed on recheck), with initial Mac queue failures retained. The physical read-only three-camera/leader workload and independent 20 x 40-second file audit PASS (99.8375% grouping; 120 Hz leader); candidate delivery is tracked in [M13](docs/m13-acceptance/lab-load-20260912.md). This does not establish physical teleoperation acceptance. Earlier lab readback/READY-interruption evidence and the historical failed Mac full-replay batch retain their original scope. The current [module matrix](docs/m13-acceptance/simulator-matrix.md) supersedes historical availability notes below.
