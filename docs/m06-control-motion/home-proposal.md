...
Status: proposed by the user; motion implementation and acceptance NOT RUN.
The user resumed offline work and proposed merging application ZERO into
READY/HOME. Prefer one named application start pose; do not reset encoders,
change joint-zero calibration, or require a separate all-zero intermediate pose.
This proposal replaces the earlier startup-zero requirement only after alignment;
it does not authorize any motion during the current read-only work.
...
additional required waypoints in this proposal.

Use the existing `ready_q_rad` field as the future single source of truth;
...
Proposed lifecycle: INITIALIZE (read-only checks) -> explicit validated
